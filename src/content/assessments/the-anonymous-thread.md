---
title: The anonymous thread
description:
  Assignment 1, due in the mid-semester break — post one small thing you
  overthought and what stopped it, anonymously, then reply to the people whose
  post could have been yours.
week: 6
due: 2027-04-09T17:00:00+10:00
weight: 20
marking:
  mode: weighted
  criteria:
    - name: Your own post
      weight: 40
    - name: Replies to others
      weight: 60
spec:
  - one anonymous post — something small you overthought, and how you got out
    of it, or honestly that you didn't
  - at least three substantive replies to other people's posts, at least one
    of which says "this happens to me too" and means it
  - posted by the deadline; your name is never attached to any of it
related:
  - the-log
  - sessions/06-how-you-stopped
heroImage: /src/assets/images/art/asmt-anonymous-thread.png
heroImageAlt: "Five people seated around a round table, one gesturing mid-sentence and another writing in a notebook — flat gold and black on cream"
---

## The brief

> Post the small thing. Then go and find the people it also happens to.

Two halves, both on the Ed forum, both anonymous.

**Your post.** One thing you overthought. Small — the kind of thing that
sounds absurd written down, which is most of them. Say what the spiral was,
and then say what got you out of it: a technique from
[How you stopped](/sessions/06-how-you-stopped/), something you invented, or
nothing at all. "I didn't get out of it, it just eventually stopped mattering"
is a complete and honest answer, and more useful to the room than a tidy one.

**Your replies.** At least three, to other people's posts, and this is the
half that carries most of the marks. Say if the same thing happens to you. Say
what you tried. Ask what you would actually want to know. The point is not
feedback — it is the discovery that the thing you assumed was your own private
malfunction has eleven other people in the thread.

## Anonymity, precisely

Posts and replies are **anonymous to other students and to the markers.**
Marking works from the forum's participation data, which tells us that an
account posted and replied, not which post was whose. So nobody grades your
content against your name.

One limit, stated plainly because you should know it before you post: a
moderator **can** identify an author if a post is reported. Anonymity here
protects you from being judged, not from the
[respect policy](/policies/#respect). A forum where nobody could ever be
identified would be a forum with no way to deal with someone being cruel in a
thread full of people admitting things, and that trade is not worth making.

You are working in a group of six for the replies, and here is the part people
ask about: **you will not be told who is in your group, and they will not be
told about you.** If you would rather work with people you know, you can opt
into a named group instead — say so in week 4 and we will arrange it. Most
people don't, which is the interesting finding of running this every year.

## Why it is due in the break, and how long you actually have

Because it should not compete with anything. The mid-semester break has no
classes, no other deadlines from us, and — for most of you — a gap before the
second half arrives. Writing one honest paragraph and reading a dozen others
is roughly a two-hour job, and a break is where a two-hour job belongs.

The date above is the **end of the first week of the break**, and it is a
soft deadline: you can take it right through to the **end of the break** with
no penalty, no form, and nobody to ask. The early date is there so that most
people are posting at the same time and the thread has something in it to
reply to; the extension is there because a break you have to work through is
not a break.

## What it turns into

The techniques from every post get collected, stripped of any identifying
detail, and put to a vote when we come back. The ones the room finds most
interesting get tried in class, one per week, for the rest of semester — see
[Crits](/sessions/) for how that works.

So this assignment is not a hoop. It is where the second half of the syllabus
comes from.

## A note on what to post

Nothing here has to be personal. The best posts are usually about a bus
timetable, an email subject line, or whether a full stop meant something. If
something is too close to write about, write about something else — that is
not evasion, it is the correct use of the pass rule, and the mark does not
know the difference.
