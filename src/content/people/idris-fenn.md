---
title: Idris Fenn
description:
  A tutor running most of the semester's studios and the empty-slot lecture,
  and marking both the audit and the final project.
affiliation: Slop University School of Invented Disciplines
role: tutor
email: idris.fenn@slop.university
contact: Ask during a studio, or email when a question cannot wait until the next one.
photo: ./idris-fenn.avif
photoAlt: A stylised portrait of a person facing the viewer, flat gold and black shapes on warm cream, in a two-ink risograph print
---

Runs "The reveal," "The sandwich," "How you stopped" and "When it helps,"
gives the week 9 lecture on incubation, and marks both assessments. Prefers
questions asked in the room where a studio just happened over questions asked
by email a week later — the second kind has usually lost the detail that made
it worth asking.
